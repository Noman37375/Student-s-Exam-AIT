...
import type { ExamResultQuestion, AnswerOption } from "@/types/exam";

interface AnswerReviewProps {
  questions: ExamResultQuestion[];
}

function getOptionText(q: ExamResultQuestion, opt: AnswerOption): string {
...
export default function AnswerReview({ questions }: AnswerReviewProps) {
  ...
  const counts = {
    ...
    incorrect: questions.filter((q) => !q.isCorrect && q.studentAnswer !== null).length,
    skipped:   questions.filter((q) => q.studentAnswer === null).length,
  };

  const filtered = questions.filter((q) => {
    if (filter === "correct")   return q.isCorrect;
    if (filter === "incorrect") return !q.isCorrect && q.studentAnswer !== null;
    if (filter === "skipped")   return q.studentAnswer === null;
    return true;
  });
                ...
                <div className="px-5 pb-5 pt-1 bg-slate-50/80 border-t border-slate-100">
                  ...
                  <div className="space-y-2">
                    {(["A", "B", "C", "D"] as AnswerOption[]).map((opt) => {
                      ...
                      return (
                        <div key={opt}
                          className={`flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm border transition-none ${
                        ...
                        >
                          <span className={`w-6 h-6 rounded-lg flex items-center justify-center text-xs font-bold flex-shrink-0 ${
                            isCorrectOpt ? "bg-emerald-200 text-emerald-700" :
                          ...
                          {isWrong && (
                            <span className="text-xs font-semibold text-red-500 flex-shrink-0">Your answer</span>
                          )}
                        </div>
                      );
                    })}
